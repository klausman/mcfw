...
if (isNull _grp) exitWith {[player] joinSilent grpNull;};

while {true} do {
    // ESTABLISH TARGET GROUP SIZE
    // We begin by establishing how many units remain alive in the group. We
    // use this to establish a baseline value to compare against.
    ...
    sleep 3;
    // CHECK PROXIMITY
    // We then check how many of the units are within the required proximity
    // to the objective; if all units qualify then we exit the loop.
