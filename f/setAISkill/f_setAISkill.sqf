...
// This are the minimal skills a soldier set to _superSkill would have. For
// all other skill levels the values are rounded using the numbers above.
// These are recommended levels to avoid "laser" AI snipers. Change them
// accordingly if you are finding the AI to be too inaccurate or are using AI
// mods.
...
// SET UP SKILL Levels
// As the skill level are passed as full integers, we interpret each of them
...
// By using the BI function BIS_fnc_MP we ensure that AI is set to the correct
// level for all connected clients, including the server

{
    ...
    if !(_x getVariable ["f_setAISkill",false]) then {
        // We change the value of skill to the appropiate one depending on the
        // unit's side
        switch (side _x) do {
        ...
        // If skill is 99 it is not configured in the params and the unit will
        // be ignored
        ...
        // Call the function to set the skills where the unit is local and
        // mark it as processed for the server
        [[_x,_skillArray],"f_fnc_setAISkill",_x,false,true] spawn BIS_fnc_MP;

        // If the unit is not local to the server, register it's skill
        // server-side as well
        if !(local _x) then {
