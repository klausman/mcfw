// Add group join option to action menu

// This script can be called either without parameters, or with a single
// boolean parameter. The boolean indicates whether players are allowed to
// join groups on different side.
...
private ["_actionDistance", "_actionGraceTime", "_actionString", "_unit", "_nearUnit", "_nearGroup", "_actionDistance", "_actionGraceTime", "_allowDifferentSide"];

// How many meters player needs to be from another group's leader for the join
// action to be shown
_actionDistance = 2.5;
// How many seconds the action is shown for when you leave group leader's
// range
_actionGraceTime = 3;

// Check if script caller wants to enable joining of groups on different
// sides, default to false
_allowDifferentSide = [_this,0,false] call bis_fnc_param;

// If nearest unit is leader of a different group with playable units in it,
// add option to join that group
while {true} do {
    if (count (nearestObjects [player, ["Man"], _actionDistance] - [player]) > 0) then {
        ...
        // Using curly braces makes the if statement cheaper to evaluate, but
        // requires OA 1.62
        if (alive _nearUnit && group player != _nearGroup && _nearUnit == leader _nearGroup && _nearUnit in playableUnits) then {
            if (_allowDifferentSide || side player == side _nearGroup) then {
                ...
                // Once player leaves the group leader's vicinity, remove
                // action
